...
export default function DarkModeSwitch({
  ...
  return (
    <label className="mr-auto inline-flex cursor-pointer items-center">
      <svg
        width="17"
        height="16"
        xmlns="http://www.w3.org/2000/svg"
        className={`mr-3 ${isChecked ? "fill-slate-500" : "fill-neutral-0"}`}
      >
      ...
      <input
        ...
        className="sr-only"
        checked={isChecked}
        onChange={() => setDarkMode(!isChecked)}
      />
      <div className="relative h-6 w-12 rounded-full bg-slate-500 transition-colors duration-300">
        <div
          className={`absolute top-0.5 left-0.5 m-1 h-3 w-3 rounded-full bg-white shadow-sm transition-transform duration-300 ${isChecked ? "translate-x-6" : ""}`}
        ></div>
      </div>
      <svg
        width="18"
        height="18"
        xmlns="http://www.w3.org/2000/svg"
        className={`ml-3 ${isChecked ? "fill-neutral-0" : "fill-slate-500"}`}
      >
