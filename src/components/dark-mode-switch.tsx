  ...
  return (
    <label className="mr-auto inline-flex cursor-pointer items-center">
      <svg
        ...
        className={`mr-3 ${checked ? "fill-slate-500" : "fill-neutral-0"}`}
      >
      ...
      <div className="relative h-6 w-12 rounded-full bg-slate-500 transition-colors duration-300">
        <div
          className={`absolute top-0.5 left-0.5 m-1 h-3 w-3 rounded-full bg-white shadow-sm transition-transform duration-300 ${checked ? "translate-x-6" : ""}`}
        ></div>
      </div>
      <svg
        width="18"
        height="18"
        xmlns="http://www.w3.org/2000/svg"
        className={`ml-3 ${checked ? "fill-neutral-0" : "fill-slate-500"}`}
      >
